...
#session_start();
$detalle = consultas::get_datos("select * from v_detalle_pedcompra where ped_cod =" . $_REQUEST['vped_cod'] . " and dep_cod=" . $_REQUEST['vdep_cod'] . " and art_cod =" . $_REQUEST['vart_cod'])
?>
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
    </button>
    <h4 class="modal-title"><i class="fa fa-edit"></i> Agregar Precio Presupuestado</h4>
</div>
<form action="prescompra_control.php" method="post" accept-charset="utf-8" class="form-horizontal">
    <div class="modal-body">
        <input type="hidden" name="accion" value="4" />
        <input type="hidden" name="vped_cod" value="<?php echo $_REQUEST['vped_cod'] ?>" />
        ...
        <div class="form-group">
            ...
            <div class="col-lg-6 col-sm-4 col-md-4 col-xs-6">
                <input type="number" name="vped_cant" class="form-control" min="1" value="<?php echo $detalle[0]['ped_cant'] ?>" required="" disabled/>
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-lg-2 col-sm-3 col-md-2 col-xs-2">Precio:</label>
            <div class="col-lg-6 col-sm-4 col-md-4 col-xs-6">
                <input type="number" name="vped_precio" class="form-control" min="1" required="" value="<?php echo $detalle[0]['ped_precio_presup'] ?>" />
            </div>
        </div>
    </div>
